...
\begin{frame}{Explanation of Transition, Reward}
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			...
			\footnotetext{\tiny where T is the decision step amount in a complete episode}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{MHA}
	...
	% 	\begin{column}{0.6\textwidth}
	% 		\centering
	% 		\includegraphics[width=\textwidth]{MHA.png} 
	% 	\end{column}
	% \end{columns}
	\begin{figure}
		\centering
...
\begin{frame}{Cooperative Multi-Agent Decoders}
	\begin{columns}
		...
		\end{column}
	\end{columns}
	\footnotetext{${\bar{h}}={\frac{1}{2N}}\sum_{i=0}^{2N}h_{i}$: The average of all nodes}
\end{frame}
