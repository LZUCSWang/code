...
\end{alignat}
   ...
   % \label{eq:ctr_shale}
    % \begin{align}
    % \max_{x_{ij}, \sigma_{i}} \ C=\sum_{i=1}^N\sum_{j\neq i}^N x_{ij}\sigma_{i}
    % \ \ \tag{\ref{eq:ctr_shale}}
    % \end{align} 
    
    % \begin{alignat}{2}
    % \text{s.t.} \ & \sum_{i=1}^N\sum_{j\neq i}^N\sigma_{i}e_{i}x_{ij}+e_{ap}\sum_{i=1}^N\sum_{j\neq i}^N\sigma_{i}x_{ij} \nonumber \\ 
    % &+ \sum_{i=0}^N\sum_{j\neq i}^N\sigma_{i}e_{i}x_{ij}(M+\overline{q}_{ij})^{\frac{3}{2}} \leq E_{max},  \\
    % &\sum_{i=1, j\neq i}^N\sigma_{i}q_{i}x_{ij} \leq Q   
