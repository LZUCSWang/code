...
assign offset = inst[15:0]; // 地址偏移量Page 114
...
assign inst_add = inst_ADDU | inst_ADDIU | inst_LW | inst_SW;// 做加法运算
...
assign inst_wdest_rd = inst_ADDU | inst_SUBU | inst_SLT | inst_AND | inst_NOR | inst_OR | inst_XOR | inst_SLL | inst_SRL;
// 寄存器堆写使能信号， 非复位状态下有效
assign rf_wen = (inst_wdest_rt | inst_wdest_rd) & resetn;
assign rf_waddr = inst_wdest_rd ? rd : rt; // 寄存器堆写地址 rd 或 rt
assign rf_wdata = inst_LW ? dm_rdata : alu_result;// 写回结果
//---------------------------{写回}end--------------------------
//
endmodule