...
class SemanticAnalyzer:
    ...
    def __init__(self):
        ...
        try:
            self.intermediate_code_temp_printer = open(
                "./gen_data/target_file/intermediate_code_temp.txt", "w"
            )
        except IOError:
            print("用来保存输出中间代码的临时文件打开失败！")

        self.next_state_num = 1  # 从1开始，0留给跳转到main的指令
        self.backpatching_level = 0  # 从0开始
        self.main_line = -1
        ...
        # self.current_symbol_table_stack_ = []

    # def __del__(self):
    #     if not self.intermediate_code_temp_printer.closed:
    #         self.intermediate_code_temp_printer.close()
    #     if not self.intermediate_code_printer_.closed:
    #         self.intermediate_code_printer_.close()
    #     if not self.intermediate_code_temp_reader_.closed:
    #         self.intermediate_code_temp_reader_.close()
    ...
    def PrintQuadruples(self, quadruples=None):
        if quadruples is not None:
            ...
            self.intermediate_code_printer_ = open(
                "./gen_data/target_file/intermediate_code.txt", "w"
            )
            if not self.intermediate_code_printer_:
                print("用来保存输出中间代码的文件打开失败！")
                return False

            self.intermediate_code_temp_reader_ = open(
                "./gen_data/target_file/intermediate_code_temp.txt", "r"
            )
            if not self.intermediate_code_temp_reader_:
                print("读取用来保存输出中间代码的临时文件打开失败！")
                return False

            # 添加第0调指令，直接跳转到Main函数所在的位置
            self.intermediate_code_printer_.write(f"0 (j, -, -, {self.main_line_})\n")

            for line in self.intermediate_code_temp_reader_:
                if line.strip():
                    self.intermediate_code_printer_.write(line)

            self.intermediate_code_temp_reader_.close()

            # 删除临时文件
            if os.path.exists("./gen_data/target_file/intermediate_code_temp.txt"):
                os.remove("./gen_data/target_file/intermediate_code_temp.txt")
    ...
    def MyExecuteSemanticCheck(self, symbol_info_stack, production):
