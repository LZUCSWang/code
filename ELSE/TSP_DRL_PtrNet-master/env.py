...
class Env_tsp():
    ...
    def get_nodes(self, seed=None, city_coordinates=None):
        '''
        ...
        # ]
        # city_coordinates = [(-191.0, -82.0), (-21.0, -353.0), (-284.0, -175.0), (-158.0, -246.0), (-41.0, -49.0), (-152.0, -36.0)]
    ...
    def stack_random_tours(self,fromCity = None):
        '''
        tour:(city_t)
        return tours:(batch,city_t)
        '''
        list = [self.get_random_tour(fromCity) for i in range(self.batch)]
    ...
    def show(self, nodes, tour, num=None):
        ...
        plt.plot(nodes[np_tour, 0], nodes[np_tour, 1], 'k-', linewidth=2.0)
        ...
        if num is not None:
            fname = 'img/img' + str(num)
            plt.savefig(fname, dpi=500, facecolor='w', edgecolor='w',
                        orientation='portrait')
        plt.show()

    # def show(self,nodes, tour, num=0):
    #     fig, ax = plt.subplots()
    #     nodes = nodes.cpu().detach()
    #     # 绘制节点
    #     ax.plot(nodes[:, 0], nodes[:, 1], 'yo', markersize=16)
    #
    #     # 绘制路径
    #     np_tour = tour.cpu().detach().numpy().tolist()
    #     np_fin_tour = [tour[-1].item(), tour[0].item()]
    #     ax.plot(nodes[np_tour, 0], nodes[np_tour, 1], 'k-', linewidth=0.7)
    #     ax.plot(nodes[np_fin_tour, 0], nodes[np_fin_tour, 1], 'k-', linewidth=0.7)
    #
    #     # 绘制箭头
    #     for i in range(len(np_tour) - 1):
    #         x = nodes[np_tour[i], 0]
    #         y = nodes[np_tour[i], 1]
    #         dx = nodes[np_tour[i + 1], 0] - x
    #         dy = nodes[np_tour[i + 1], 1] - y
    #         ax.annotate("", xy=(nodes[np_tour[i + 1], 0], nodes[np_tour[i + 1], 1]), xytext=(x, y),
    #                     arrowprops=dict(arrowstyle="->", linewidth=1.5), color='r')
    #
    #     # 添加节点标签
    #     for i in range(len(nodes)):
    #         ax.text(nodes[i, 0], nodes[i, 1], str(i), size=10, color='b')
    #
    #     # 保存图像
    #     if num > 0:
    #         fname = 'img/img' + str(num)
    #         plt.savefig(fname, dpi=500, facecolor='w', edgecolor='w', orientation='portrait')
    #
    #     plt.show()
    ...
    def get_random_tour(self, fromCity = None):
        '''
        return tour:(city_t)
        '''
        tour = []
