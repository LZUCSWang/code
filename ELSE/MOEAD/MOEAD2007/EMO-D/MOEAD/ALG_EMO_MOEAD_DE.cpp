...
void CALG_EMO_MOEAD_DE::InitializePopulation()
{
	...
	for(i=0; i<s_PopulationSize; i++)
	{
		...
		SP.m_BestIndividual.Evaluate();					//计算函数值
		...
		{
			readf>>SP.v_Weight_Vector[j];				//载入权重向量
		}

		m_PopulationSOP.push_back(SP);					//将子问题push进表中
	}
	readf.close();
}
...
void CALG_EMO_MOEAD_DE::EvolvePopulation()
{
	...
	UtilityToolBox.RandomPermutation(order, 0);				//打乱order
	...
	for(unsigned int s=0; s<s_PopulationSize; s++)		//遍历order
	{
